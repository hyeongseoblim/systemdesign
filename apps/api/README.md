...
수동 카드의 소스는 `src/main/resources/content/*.md`이며, 부팅 시 `ContentSeeder`가 slug 기준으로 멱등 적재한다. 소스 기준 현재 129개 카드가 있다. 선택적인 `topicKey` 프론트매터를 넣으면 해당 커리큘럼 주제가 `MANUAL` 상태로 연결되어 AI 중복 생성을 막는다.
