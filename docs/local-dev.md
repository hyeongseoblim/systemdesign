...
2. **ContentSeeder** 가 `apps/api/src/main/resources/content/*.md` 를 slug 기준 멱등으로 적재 — 학습 카드 71개가 발행(`source=MANUAL`) 상태로 들어간다. 이 중 `topicKey`가 있는 12개는 커리큘럼을 `MANUAL`로 해결하므로, 신규 DB 부팅 완료 후 AI 생성 대기는 357개다. 재기동해도 중복 생성되지 않는다.
...
| `GEN_ENABLED` | `false` | 생성 배치 on/off. Cloud Run scale-to-zero 환경에서는 Cloud Scheduler를 구성하기 전까지 `false`로 유지 |
